...
export interface CodeSnippet {
  ...
  line: number;        // 1-based line number of the keyword match
  keyword: string;
  context: string;     // matched line ± CONTEXT_LINES lines
}
...
export interface Hypothesis {
  id: string;
  pattern: string;     // e.g. "inverted_comparison"
...
// ─────────────────────────────────────────────────────────────────────────────
// STATIC BUG PATTERN DETECTORS (Approach 3)
// ─────────────────────────────────────────────────────────────────────────────

interface Detector {
  id: string;
  confidence: "high" | "medium" | "low";
  /** Returns a description if the line matches this pattern, null otherwise */
  test(line: string, context: string): string | null;
}
...
const DETECTORS: Detector[] = [
  {
    ...
    test(line) {
      ...
      if (!hasDomainWord) return null;
      const m = line.match(/([><=!]+)/);
      if (!m) return null;
      const op = m[1];
      const inv: Record<string, string> = { ">=": "<", "<=": ">", ">": "<=", "<": ">=" };
      if (inv[op]) return `Comparison '${op}' may be inverted — should it be '${inv[op]}'?`;
      return null;
    },
  },
  {
    ...
    confidence: "high",
    test(line, context) {
      if (!line.includes(".get()")) return null;
      if (/orElse|orElseThrow|ifPresent|isPresent|isEmpty|map\(/.test(context)) return null;
  ...
  },
  {
    ...
    confidence: "medium",
    test(line) {
      if (/return\s+null\b/.test(line)) return "Method returns null — caller may not handle it";
      return null;
    },
  },
  {
    ...
    confidence: "medium",
    test(line, context) {
      if (!(/\.get\(0\)/.test(line) || /\.getFirst\(\)/.test(line) || /\.first\(\)/.test(line))) return null;
      if (/isEmpty|size\(\)|!empty/i.test(context)) return null;
  ...
  },
  {
    ...
    test(line, context) {
      ...
      if (!isPublicModifying) return null;
      const modifiesData = /save\(|delete\(|update\(|persist\(|merge\(/.test(context);
      if (!modifiesData) return null;
      if (/@Transactional/.test(context)) return null;
  ...
  },
  {
    ...
    test(line) {
      ...
      if (/WHERE\s+\w+\s*[><=!]+\s*0\b/.test(line)) return `WHERE clause compares against literal 0 — may be using wrong field`;
      if (/WHERE\s+\w+\s*=\s*null\b/i.test(line)) return `WHERE clause compares against null — use IS NULL instead`;
      return null;
    },
  },
];
...
export class CodeBriefBuilder {
   ...
   */
  async build(issue: string): Promise<CodeBrief> {
    const t0 = Date.now();
    ...
    for (const [rawUrl] of urlMatches) {
      ...
      for (const seg of [...segments].reverse()) {
        ...
        const endpointSnippets = (await Promise.allSettled(
          this.sourceRoots.map(dir => this.grepKeyword(annotationPattern, dir))
    ...
    for (const kw of topKeywords) {
      if (allSnippets.length >= MAX_SNIPPETS) break;
      const perRootResults = await Promise.allSettled(
        this.sourceRoots.map(dir => this.grepKeyword(kw, dir))
      );
      let kwCount = 0;
      for (const r of perRootResults) {
        if (kwCount >= MAX_PER_KW_GLOBAL) break;
        if (r.status !== "fulfilled") continue;
        for (const s of r.value) {
          if (kwCount >= MAX_PER_KW_GLOBAL) break;
          const key = `${s.file}:${s.line}`;
          if (!seen.has(key)) { seen.add(key); allSnippets.push(s); kwCount++; }
          if (allSnippets.length >= MAX_SNIPPETS) break;
        }
        if (allSnippets.length >= MAX_SNIPPETS) break;
      }
    ...
    const brief: CodeBrief = {
      ...
      tokenEstimate: Math.ceil(this.formatBriefForPrompt(
        { keywords: topKeywords, snippets: allSnippets, hypotheses, buildTimeMs: 0, tokenEstimate: 0 }
      ).length / 4),
    };
  ...
  async extractMethod(
    ...
    const methodRe = new RegExp(
      `(?:^|\\s)(?:public|private|protected|async|static|override|abstract)?\\s*(?:async\\s+)?${escapeRegex(methodName)}\\s*[\\(<]`,
      "m"
    );
    ...
    for (let i = startIdx - 1; i >= 0; i--) {
      ...
      if (trimmed.startsWith("@") || trimmed === "") { blockStart = i; }
      else break;
    }
    ...
    for (let i = startIdx; i < Math.min(startIdx + 200, lines.length); i++) {
      ...
      if (inBody && depth === 0) { endIdx = i; break; }
    }
   ...
   */
  async verifyPattern(searchStrategy: string[]): Promise<PatternVerification> {
  ...
  formatBriefForPrompt(brief: CodeBrief): string {
    if (brief.snippets.length === 0) return "";
   ...
   */
  private async grepKeyword(keyword: string, dir: string): Promise<CodeSnippet[]> {
    ...
    for (const matchLine of matchLines) {
      if (snippets.length >= MAX_PER_KW) break;
  ...
  private async findFileInSourceRoots(fileName: string): Promise<{ absPath: string; relPath: string } | null> {
    for (const root of this.sourceRoots) {
      try {
        const { stdout } = await execAsync(
          `find "${root}" -name "${fileName}" -type f 2>/dev/null | head -1`,
          { timeout: GREP_TIMEOUT }
        );
        const absPath = stdout.trim();
        if (absPath) return { absPath, relPath: path.relative(this.projectRoot, absPath) };
  ...
  private async extractContext(absFile: string, lineNum: number): Promise<string | null> {
    try {
   ...
   */
  private minifyCode(contextWithLineNumbers: string): string {
    ...
    for (const line of lines) {
      ...
      if (commentIdx !== -1) code = code.slice(0, commentIdx);

      code = code.trimEnd();
      if (!code.trim()) continue;  // skip blank / comment-only lines
  ...
  private generateHypotheses(snippets: CodeSnippet[]): Hypothesis[] {
  ...
  extractKeywords(issue: string): string[] {
    const seen  = new Set<string>();
    const valid = (w: string) => w.length >= 3 && !STOP_WORDS.has(w) && !/^\d+$/.test(w);
    const add   = (words: string[]) => words.forEach(w => { if (valid(w)) seen.add(w); });
