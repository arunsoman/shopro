 ...
 */
import { AuthStorage, createAgentSession, DefaultResourceLoader, ModelRegistry, SessionManager, createCodingTools, } from "@mariozechner/pi-coding-agent";
...
class Orchestrator {
    ...
    async runTask(taskDescription, maxRetries = DEFAULT_MAX_RETRIES) {
...
`;
    ...
    async buildRetryContext(qaReport) {
    ...
    async extractFilesFromOutput(output, type) {
    ...
    async saveTaskState() {
        if (!this.taskState)
            return;
        await fs.writeFile(path.join(STATE_DIR, "task-state.json"), JSON.stringify(this.taskState, null, 2));
    }
