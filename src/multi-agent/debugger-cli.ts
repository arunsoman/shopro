...
import type { CodeBrief, MethodSnippet } from "./code-brief-builder.js";
...
/** Stored example of a previous fix for this class of bug (used in EstimatedContext) */
interface PastFixExample {
  oldFile: string;
  oldMethod: string;
  /** Key lines changed in the remediation, extracted from remediation output */
  oldDiff: string;
  /** The Estimated Context block that was injected when the fix was found */
  estimatedContextUsed: string;
}
...
interface PatternMatchResult {
  pattern: DebugPattern | null;
  score: number;
  matchedKeywords: string[];
}

/**
 * Context block to inject into Phase 2 prompt.
 * Either rule-based adaptation from a matched pattern (score 0.80–0.84)
 * or a git-blame snapshot when no pattern matches at all.
 */
interface EstimatedContext {
  type: "pattern-match" | "git-blame-fallback";
  confidence: number;
  patternName?: string;
  block: string;
}

/** Cached result of a Phase 2 investigation run */
...
function snapshotTokens(
  session: any,
  phase: string,
  baseline?: { input: number; output: number; cacheRead: number; total: number }
): TokenPhase {
...
function printTokenTable(ledger: TokenLedger): void {
  if (ledger.length === 0) return;
  const totals = ledger.reduce(
    (acc, p) => ({ input: acc.input + p.input, output: acc.output + p.output, cacheRead: acc.cacheRead + p.cacheRead, total: acc.total + p.total }),
    { input: 0, output: 0, cacheRead: 0, total: 0 }
  );
  const fmt = (n: number) => n.toLocaleString().padStart(9);
  console.log("\n" + "─".repeat(62));
...
function attachOutputCapture(session: any, verbose = false): () => string {
  let textOutput = "";
    ...
    if (event.type === "message_update") {
      ...
      switch (ae.type) {
        case "text_delta":
          ...
          if (ae.contentIndex !== undefined) streamedIndexes.add(ae.contentIndex as number);
          break;
        case "text_end":
          if (ae.content && (ae.contentIndex === undefined || !streamedIndexes.has(ae.contentIndex as number))) {
            textOutput += ae.content;
            process.stdout.write(ae.content);
          }
          break;
        case "thinking_delta":
          if (verbose && ae.delta) process.stdout.write(`\x1b[2m${ae.delta}\x1b[0m`);
          break;
        case "thinking_end":
      ...
      if (text) { textOutput = text; console.log(text); }
    }
  });
...
function isPlaceholderReport(obj: Record<string, unknown>): boolean {
...
function extractJson<T>(output: string, anchorKey: string): T | null {
  const candidates: Array<() => T | null> = [
    () => {
      const all = matchAll(/```json\s*\n?([\s\S]*?)\n?```/i, output);
      if (!all.length) return null;
      return JSON.parse(all[all.length - 1][1].trim()) as T;
    },
    () => {
    ...
    () => {
      ...
      if (!all.length) return null;
      return JSON.parse(all[all.length - 1][0]) as T;
    },
    () => {
      const first = output.indexOf("{"), last = output.lastIndexOf("}");
      return first !== -1 && last > first ? JSON.parse(output.slice(first, last + 1)) as T : null;
    },
  ];
  ...
  for (const tryParse of candidates) {
    try {
      ...
      if (result && !isPlaceholderReport(result as Record<string, unknown>)) return result;
    } catch { /* fall through */ }
  }
  return null;
}
...
async function loadPatterns(): Promise<PatternStore> {
  try { return JSON.parse(await fs.readFile(PATTERNS_FILE, "utf-8")); }
  catch { return { version: 1, patterns: [], lastUpdated: new Date().toISOString() }; }
}

async function savePatterns(store: PatternStore): Promise<void> {
...
function patternEmbedText(p: DebugPattern): string {
 ...
 */
function keywordRawScore(issueTokens: string[], p: DebugPattern): { rawScore: number; matched: string[] } {
 ...
 */
async function matchPattern(issue: string, patterns: DebugPattern[]): Promise<PatternMatchResult> {
  const candidates  = patterns.filter(p => p.confidence >= 0.20);
  if (candidates.length === 0) return { pattern: null, score: 0, matchedKeywords: [] };

  const issueVec    = await embedText(issue);
  const issueTokens = tokenise(issue);

  let best: DebugPattern | null = null, bestScore = 0, bestMatched: string[] = [];
  ...
  for (const p of candidates) {
    ...
    if (score > bestScore) { bestScore = score; best = p; bestMatched = matched; }
  }

  return { pattern: bestScore >= MATCH_THRESHOLD ? best : null, score: bestScore, matchedKeywords: bestMatched };
}

function updatePatternStats(store: PatternStore, id: string, succeeded: boolean): void {
 ...
 */
async function probeOpenRouterModel(modelId: string): Promise<boolean> {
  try {
...
async function preFlightChecks(): Promise<{ model: any; authStorage: any; modelRegistry: any }> {
  console.log("\n🔧 PRE-FLIGHT");
  ...
  if (!model) {
    ...
    if (!model) {
      throw new Error(
        "Ollama is unreachable AND all OpenRouter free models are rate-limited.\n" +
...
function computeCacheKey(issue: string, brief: CodeBrief, patternId?: string): string {
  const input = [
    issue.trim(),
    brief.keywords.join(","),
...
async function loadCachedReport(hash: string): Promise<CacheEntry | null> {
  try {
...
async function saveCachedReport(hash: string, report: DebugReport): Promise<void> {
 ...
 */
function generateEstimatedContext(
 ...
 */
async function getGitBlameFallback(issue: string): Promise<EstimatedContext | null> {
  ...
  try {
    const { stdout: findOut } = await execAsync(
      `find "${PROJECT_ROOT}/shopro-res/src" "${PROJECT_ROOT}/shopro-res-web/src" -name "${fileName}" -type f 2>/dev/null | head -1`,
      { timeout: 6000 }
    );
    ...
    const { stdout: blameOut } = await execAsync(
      `git -C "${PROJECT_ROOT}" blame -L ${lineNum},${lineNum} -- "${relPath}" 2>/dev/null`,
      { timeout: 5000 }
    );
    const commitHash = blameOut.trim().split(" ")[0];
    if (!commitHash || /^0+$/.test(commitHash)) return null;

    const { stdout: diffOut } = await execAsync(
      `git -C "${PROJECT_ROOT}" show ${commitHash} -- "${relPath}" 2>/dev/null | head -50`,
      { timeout: 5000 }
    );
...
async function tryFastTrack(
  _issue: string,
  pattern: DebugPattern,
  brief: CodeBriefBuilder
  ...
  return {
    ...
    category:        pattern.category as DebugReport["category"],
...
async function runInvestigation(
  ...
  brief: CodeBrief,
  briefBuilder: CodeBriefBuilder,
  estimatedContext: EstimatedContext | null,
  model: any,
  authStorage: any,
  modelRegistry: any,
  verbose: boolean
): Promise<{ rawOutput: string; report: DebugReport | null; session: any; tokens: TokenPhase }> {

  const agentsFile = path.join(AGENTS_DIR, "debugger", "AGENTS.md");
  let agentsContent = "";
  try { agentsContent = await fs.readFile(agentsFile, "utf-8"); } catch { /* no AGENTS.md */ }
  ...
  let report    = extractJson<DebugReport>(rawOutput, "rootCause");
  if (!report) {
    console.log("   ↩️  No JSON in first response — requesting JSON output...");
    await session.prompt(
      "Your analysis above is complete. Now output ONLY the DebugReport JSON block. " +
      "No prose, no tool calls. Just the ```json block with all fields filled in."
    );
    rawOutput = getOutput();
    report    = extractJson<DebugReport>(rawOutput, "rootCause");
  }
...
function buildInvestigationPrompt(
  ...
  brief: CodeBrief,
  briefBuilder: CodeBriefBuilder,
  mode: "hypothesis_confirm" | "assisted" | "full",
  estimatedContext: EstimatedContext | null
): string {
...
async function runSpecialistAgent(
  ...
  modelRegistry: any,
  verbose: boolean
): Promise<{ output: string; filesChanged: FileChange[]; methodSnippets: MethodSnippet[]; tokens: TokenPhase }> {

  // Approach 2: pre-extract affected methods before the LLM session starts
  const methodSnippets: MethodSnippet[] = [];
  for (const file of report.affectedFiles) {
    for (const method of report.affectedMethods) {
      ...
      if (snippet) methodSnippets.push(snippet);
    }
  }

  const agentsFile = path.join(AGENTS_DIR, agentId, "AGENTS.md");
  let agentsContent = "";
  try { agentsContent = await fs.readFile(agentsFile, "utf-8"); }
  catch { console.warn(`   ⚠️  No AGENTS.md for ${agentId}`); }
  ...
  await fs.writeFile(
    path.join(STATE_DIR, `${agentId}-output.json`),
    JSON.stringify({ agentId, issue, report, output, filesChanged, completedAt: new Date().toISOString() }, null, 2)
  );
...
async function runTesterAgent(
  ...
  modelRegistry: any,
  verbose: boolean
): Promise<{ result: VerificationResult; tokens: TokenPhase }> {
  ...
  let agentsContent = "";
  try { agentsContent = await fs.readFile(agentsFile, "utf-8"); } catch { /* no file */ }
...
`;
  ...
  const parsed = extractJson<Partial<VerificationResult>>(output, "testsCreated");
  ...
  await fs.writeFile(
    path.join(STATE_DIR, "tester-output.json"),
    JSON.stringify({ issue, report, result, completedAt: new Date().toISOString() }, null, 2)
  );
...
async function runQAAgent(
  issue: string,
  report: DebugReport,
  testerResult: VerificationResult,
  model: any,
  authStorage: any,
  modelRegistry: any,
  verbose: boolean
): Promise<{ result: QAResult; tokens: TokenPhase }> {
...
`;
  ...
  const parsed = extractJson<Partial<QAResult>>(output, "status");
  ...
  await fs.writeFile(
    path.join(STATE_DIR, "qa-output.json"),
    JSON.stringify({ issue, report, result, completedAt: new Date().toISOString() }, null, 2)
  );

  await fs.writeFile(
    path.join(REPORTS_DIR, `qa-report-${timestamp}.json`),
    JSON.stringify({
...
async function learnPattern(
  ...
  remediationOutput: string,
  estimatedContext: EstimatedContext | null
): Promise<{ patternId: string | null; tokens: TokenPhase }> {

  const noTokens = (): TokenPhase => ({ phase: "PATTERN_LEARNING", input: 0, output: 0, cacheRead: 0, total: 0 });
  ...
  await session.prompt(
    `Distil this debug session into a reusable pattern. Respond with ONLY a JSON block:
...
Context: issue="${issue.slice(0, 100)}", rootCause="${report.rootCause}", fix="${report.recommendedFix}"`
  ...
  const raw       = getOutput();
  const extracted = extractJson<Partial<DebugPattern>>(raw, "name");
 ...
 */
function extractDiffSummary(remediationOutput: string): string {
...
function extractFileChanges(output: string): FileChange[] {
...
function startPhase(session: DebugSession, name: string, agentId?: string): PhaseResult {
  const phase: PhaseResult = { name, status: "RUNNING", startedAt: new Date().toISOString(), ...(agentId ? { agentId } : {}) };
  session.phases.push(phase);
  return phase;
}

function endPhase(phase: PhaseResult, status: "SUCCESS" | "FAILED" | "SKIPPED", note?: string): void {
  phase.status = status;
...
`);
  ...
  if (args.includes("--patterns")) {
    ...
    if (!store.patterns.length) { console.log("\nNo patterns stored yet.\n"); return; }
  ...
  for (let i = 0; i < args.length; i++) {
    if      (args[i] === "--issue"   || args[i] === "-i") { issue = args[++i] ?? ""; }
    else if (args[i] === "--file"    || args[i] === "-f") { issue = await fs.readFile(path.resolve(PROJECT_ROOT, args[++i] ?? ""), "utf-8"); }
    else if (args[i] === "--verbose" || args[i] === "-v") { verbose = true; }
    else if (!args[i].startsWith("-"))                    { issue = args[i]; }
  }

  if (!issue.trim()) { console.error("Error: no issue provided. Use --help."); process.exit(1); }
  ...
  try {
    ...
    let estimatedContext: EstimatedContext | null = null;
    ...
    let investigationSession: any  = null;
    ...
    if (!report) {
      const result = await runInvestigation(
        issue, match, brief, briefBuilder, estimatedContext,
        model, authStorage, modelRegistry, verbose
      );
      ...
      await fs.writeFile(
        path.join(STATE_DIR, "debugger-output.json"),
        JSON.stringify({ sessionId: debugSession.sessionId, issue, report, rawOutput: result.rawOutput, completedAt: new Date().toISOString() }, null, 2)
      );
      ...
      if (report) {
        ...
        if (match.pattern) { updatePatternStats(store, match.pattern.id, false); await savePatterns(store); }
    ...
    if (report) {
      ...
      if (cachedHit) console.log(`   Source     : cache [${cacheKey}]`);
    ...
    if (report) {
      ...
      const {
        ...
        filesChanged,
        tokens: remTokens,
      } = await runSpecialistAgent(agentId, issue, report, briefBuilder, model, authStorage, modelRegistry, verbose);
      ...
      const { result: verResult, tokens: verTokens } = await runTesterAgent(
        issue, report, remOutput, briefBuilder, model, authStorage, modelRegistry, verbose
      );
      ...
      const { result: qaResult, tokens: qaTokens } = await runQAAgent(
        issue, report, verResult, model, authStorage, modelRegistry, verbose
      );
      ...
      const { patternId: learnedId, tokens: learnTokens } = await learnPattern(
        issue, report, filesChanged, debugSession.sessionId,
        model, authStorage, modelRegistry,
        report.patternId ?? null, store,
    ...
    console.error("\n❌ Failed:", err);
    await fs.writeFile(
      path.join(STATE_DIR, `debug-error-${debugSession.sessionId}.json`),
      JSON.stringify({ sessionId: debugSession.sessionId, issue, error: String(err), phases: debugSession.phases }, null, 2)
    );
    process.exit(1);
  }
}
