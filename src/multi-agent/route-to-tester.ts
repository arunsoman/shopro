...
async function readJsonState(filename: string): Promise<any> {
  try {
...
async function main() {
  ...
  try {
    ...
    const model =
      modelRegistry.find("openrouter", "google/gemma-4-31b-it:free") ??
    ...
    if (beOutput) {
      ...
      const files = beOutput.filesChanged
        ?.map((f: any) => `- ${f.file}${f.change ? `: ${f.change}` : ""}`)
        .join("\n") ?? "";
      beContext = `
...
${r.evidence?.map((e: string) => `- ${e}`).join("\n")}`;
    ...
    const prompt =
`# Task: Write Tests for the Recent BE Fix
${beContext}
${debuggerContext}
