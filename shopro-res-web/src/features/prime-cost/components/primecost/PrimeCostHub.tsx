...
import { ErrorState } from "../ui/shared"
import { Money, Pct } from "../ui/shared"
...
export default function PrimeCostHub({ restaurantId }: { restaurantId: number }) {
  ...
  const primeCostPct = liveData?.primeCostPct ?? 0
  const status = live.isLoading ? "ok" : primeCostPct > 0.65 ? "critical" : primeCostPct > 0.60 ? "warn" : "ok"
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-8 bg-slate-50/20 dark:bg-transparent">
          <div className="max-w-5xl mx-auto space-y-8 pb-10">
            ...
            <Suspendable
              ...
              skeleton={
                <div className="rounded-2xl p-6 md:p-8 border border-slate-200 dark:border-white/5 bg-slate-50/50 dark:bg-black/20 animate-pulse">
                  <div className="h-3 w-40 bg-muted/20 rounded mb-4" />
                  <div className="h-14 w-32 bg-muted/20 rounded mb-4" />
              ...
              skeleton={
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-pulse">
                  {Array.from({ length: 4 }).map((_, i) => (
                    <div key={i} className="bg-white dark:bg-slate-800/20 border border-slate-200 dark:border-white/5 rounded-2xl p-4 space-y-2">
