...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query("SELECT MAX(o.createdAt) FROM Order o WHERE o.restaurantId = :restaurantId")
    LocalDateTime findMaxCreatedAt(@Param("restaurantId") Long restaurantId);

    @Query("SELECT SUM(o.totalAmount) FROM Order o WHERE o.restaurantId = :restaurantId AND o.createdAt >= :start AND o.status = 'PAID'")
    BigDecimal sumTotalSalesAfter(@Param("restaurantId") Long restaurantId, @Param("start") LocalDateTime start);

    @Query("SELECT SUM(o.session.guestCount) FROM Order o WHERE o.restaurantId = :restaurantId AND o.createdAt >= :start AND o.status = 'PAID'")
    Integer sumCoversAfter(@Param("restaurantId") Long restaurantId, @Param("start") LocalDateTime start);

    @Query("SELECT o FROM Order o WHERE o.restaurantId = :restaurantId AND o.createdAt >= :start AND o.status = 'PAID'")
    List<Order> findAllPaidAfter(@Param("restaurantId") Long restaurantId, @Param("start") LocalDateTime start);

    @Query("SELECT SUM(o.totalAmount) FROM Order o WHERE o.restaurantId = :restaurantId AND o.createdAt >= :start AND o.createdAt <= :end AND o.status = 'PAID'")
    ...
    @Query("SELECT SUM(o.session.guestCount) FROM Order o WHERE o.restaurantId = :restaurantId AND o.createdAt >= :start AND o.createdAt <= :end AND o.status = 'PAID'")
    ...
    @Query("SELECT SUM(l.quantity * COALESCE(m.plateCost, 0.0)) " +
           "FROM Order o JOIN o.lines l JOIN l.menuItem m " +
           "WHERE o.restaurantId = :restaurantId " +
           "AND o.createdAt >= :start AND o.createdAt <= :end")
    BigDecimal computeTheoreticalCos(
            @Param("restaurantId") Long restaurantId, 
            @Param("start") LocalDateTime start, 
            @Param("end") LocalDateTime end);
    ...
    @Transactional
    @Query("DELETE FROM Order o WHERE o.restaurantId = :restaurantId")
