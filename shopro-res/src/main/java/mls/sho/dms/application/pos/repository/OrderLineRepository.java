...
public interface OrderLineRepository extends JpaRepository<OrderLine, Long> {
    ...
    @Modifying
    @Transactional
    @Query("DELETE FROM OrderLine l WHERE l.order.id IN (SELECT o.id FROM Order o WHERE o.session.table.restaurant.id = :restaurantId)")
    void deleteByOrderSessionTableRestaurantId(@Param("restaurantId") Long restaurantId);

    @Query("SELECT l FROM OrderLine l WHERE l.order.session.table.restaurant.id = :restaurantId AND l.order.createdAt BETWEEN :start AND :end")
    ...
    @Query("SELECT l.menuItem.id, SUM(l.quantity), AVG(l.unitPrice) " +
           "FROM OrderLine l " +
           "WHERE l.order.session.table.restaurant.id = :restaurantId " +
