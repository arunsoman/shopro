...
public class TablesideServiceImpl implements TablesideService {
    ...
    public TablesideSessionDto createOrGetSession(UUID tableId) {
        return sessionRepo.findByTableIdAndStatus(tableId, TablesideSessionStatus.ACTIVE)
            ...
            .orElseGet(() -> {
                ...
                // Default to PENDING_APPROVAL for security
                newSession.setStatus(TablesideSessionStatus.PENDING_APPROVAL);
                
                TablesideSession saved = sessionRepo.save(newSession);
                broadcastPendingSession(saved);
                return toDto(saved);
            });
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public TableQrResponse getTableQrCode(UUID tableId) {
        TableShape table = tableRepo.findById(tableId)
            .orElseThrow(() -> new IllegalArgumentException("Table not found"));
        
        TablesideSession session = sessionRepo.findByTableIdAndStatus(tableId, TablesideSessionStatus.ACTIVE)
            .orElseGet(() -> {
                TablesideSession newSession = new TablesideSession();
                newSession.setTable(table);
                newSession.setQrToken(UUID.randomUUID());
                newSession.setStatus(TablesideSessionStatus.ACTIVE); // Codes generated by admin are pre-activated? 
                // Actually, US-1.1 says scanning directs to menu. 
                // Let's stick to the token in the URL.
    ...
    @Override
    @Transactional(readOnly = true)
    public List<TableQrResponse> getAllTableQrCodes() {
        return tableRepo.findAll().stream()
            .map(table -> getTableQrCode(table.getId()))
            .collect(Collectors.toList());
    }

    @Override
    @Transactional
    public void approveSession(UUID sessionId) {
        TablesideSession session = sessionRepo.findById(sessionId)
            .orElseThrow(() -> new IllegalArgumentException("Session not found"));
        session.setStatus(TablesideSessionStatus.ACTIVE);
        sessionRepo.save(session);
        // Notify guest app via WebSocket (topic per session or per table)
        messagingTemplate.convertAndSend("/topic/tableside/session/" + session.getId(), "APPROVED");
    }
    ...
    public void rejectSession(UUID sessionId) {
        TablesideSession session = sessionRepo.findById(sessionId)
            .orElseThrow(() -> new IllegalArgumentException("Session not found"));
        session.setStatus(TablesideSessionStatus.REJECTED);
        sessionRepo.save(session);
        messagingTemplate.convertAndSend("/topic/tableside/session/" + session.getId(), "REJECTED");
    }

    @Override
    @Transactional(readOnly = true)
    public List<TablesideSessionDto> getPendingSessions() {
        // Need a method in repository for this
        return sessionRepo.findAllByStatus(TablesideSessionStatus.PENDING_APPROVAL).stream()
            .map(this::toDto)
            .collect(Collectors.toList());
    }
    ...
    @Override
    @Transactional
    public void invalidateSessionsForTable(UUID tableId) {
        List<TablesideSession> activeSessions = sessionRepo.findAllByTableIdAndStatus(tableId, TablesideSessionStatus.ACTIVE);
        activeSessions.addAll(sessionRepo.findAllByTableIdAndStatus(tableId, TablesideSessionStatus.PENDING_APPROVAL));
        
        for (TablesideSession session : activeSessions) {
            session.setStatus(TablesideSessionStatus.EXPIRED);
            messagingTemplate.convertAndSend("/topic/tableside/session/" + session.getId(), "EXPIRED");
        }
        sessionRepo.saveAll(activeSessions);
    }

    private void broadcastPendingSession(TablesideSession session) {
        messagingTemplate.convertAndSend("/topic/tableside/pending", toDto(session));
    }
