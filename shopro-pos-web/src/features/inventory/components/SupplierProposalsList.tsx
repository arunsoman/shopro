...
export const SupplierProposalsList: React.FC = () => {
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'ACCEPTED':
                return <Badge className="bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/40 dark:text-emerald-400">{t('common.statuses.accepted')}</Badge>;
            case 'REJECTED':
                return <Badge className="bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-900/40 dark:text-rose-400">{t('common.statuses.rejected')}</Badge>;
            default:
                return <Badge className="bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-900/40 dark:text-amber-400">{t('common.statuses.pending')}</Badge>;
        }
    };
            ...
            <div className="grid gap-4">
                {!proposals || proposals.length === 0 ? (
                    <Card className="border-dashed border-2 py-16 text-center text-slate-500 bg-transparent">
                        ...
                        <p>{t('inventory.proposals.empty')}</p>
                        <p className="text-xs">{t('inventory.proposals.emptyHint')}</p>
                    </Card>
                ) : (
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="flex flex-wrap gap-4 text-sm text-slate-500">
                                            <div className="flex items-center gap-1.5 font-medium text-emerald-600 dark:text-emerald-400">
                                                {t('inventory.proposals.labels.proposed', { 
                                                    symbol: t('common.currencySymbol'), 
                                                    price: proposal.proposedPrice.toFixed(2),
                                                    unit: t(`common.units.${proposal.unitOfMeasure}`, proposal.unitOfMeasure)
                                                })}
                                            </div>
                                            <div className="text-slate-400">
                                                {t('inventory.proposals.labels.current', { 
                                                    symbol: t('common.currencySymbol'), 
                                                    price: proposal.currentPrice.toFixed(2) 
                                                })}
                                            </div>
                                            <div className="flex items-center gap-1.5 border-l pl-4">
                                                <Clock className="h-4 w-4" />
                                                {t('inventory.proposals.labels.submitted', { 
                                                    date: format(new Date(proposal.createdAt), 'MMM d, HH:mm') 
                                                })}
                                            </div>
                                        </div>
                                        {proposal.notes && (
                                        ...
                                        {proposal.generatedPoId ? (
                                            <Button variant="outline" className="gap-2 border-emerald-200 hover:bg-emerald-50 dark:hover:bg-emerald-950/20" asChild>
                                                <Link to={`/supplier/po/${proposal.generatedPoId}`}>
                                                    ...
                                                    {t('inventory.proposals.actions.viewPo')}
                                            ...
                                            <div className="text-xs text-amber-600 flex items-center gap-1 bg-amber-50 dark:bg-amber-900/20 px-2 py-1 rounded">
                                                ...
                                                {t('inventory.proposals.poPending')}
                                            </div>
                                        ) : null}
