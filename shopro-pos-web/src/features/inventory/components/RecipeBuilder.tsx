...
export const RecipeBuilder: React.FC<RecipeBuilderProps> = ({ menuItemId, menuItemName }) => {
    const { t } = useTranslation();
    const { data: ingredients } = useIngredients();
    const { data: subRecipes } = useSubRecipes();
    const { data: existingRecipe } = useRecipe(menuItemId);

    const [localIngredients, setLocalIngredients] = useState<RecipeIngredient[]>([]);
    const [totalCost, setTotalCost] = useState(0);

    const updateRecipe = useUpdateRecipe(menuItemId);

    useEffect(() => {
        if (existingRecipe?.ingredients && ingredients && subRecipes) {
            const mapped = (existingRecipe.ingredients as any[]).map(backendItem => {
                // Backend often sends everything as ingredientId; check if it's actually a sub-recipe
                const isSubRecipe = subRecipes.some(s => s.id === backendItem.ingredientId);
                return {
                    ...backendItem,
                    ingredientId: isSubRecipe ? undefined : backendItem.ingredientId,
                    subRecipeId: isSubRecipe ? backendItem.ingredientId : undefined,
                    // Map effectiveCost from backend to cost for frontend
                    cost: backendItem.effectiveCost || backendItem.cost
                } as RecipeIngredient;
            });
            setLocalIngredients(mapped);
        }
    }, [existingRecipe, ingredients, subRecipes]);
    ...
    const handleSave = () => {
        ...
        updateRecipe.mutate({
            ...
            onSuccess: () => {
                toast.success(t('inventory.recipes.success', { 
                    symbol: t('common.currencySymbol'), 
                    cost: totalCost.toFixed(2) 
                }));
            },
            onError: () => {
                toast.error(t('inventory.recipes.error'));
            }
        });
    };
    ...
    return (
        <Card className="w-full">
            <CardHeader className="flex flex-row items-center justify-between">
                <div>
                    <CardTitle>{t('inventory.recipes.builderTitle', { name: menuItemName })}</CardTitle>
                    <div className="text-sm text-muted-foreground mt-1">
                        {t('inventory.recipes.builderDesc')}
                    </div>
                </div>
                <div className="text-right">
                    <div className="text-sm font-medium">{t('inventory.recipes.estFoodCost')}</div>
            ...
            <CardContent className="space-y-4">
                                ...
                                <div className="flex-1 space-y-1">
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => {
                                            ...
                                            updateIngredient(index, isSub ? 'subRecipeId' : 'ingredientId', val);
                                        }}
                                    >
                ...
                <div className="flex justify-between items-center pt-4">
                    ...
                    <Button onClick={handleSave} disabled={updateRecipe.isPending} className="gap-2">
                        ...
                        {t('inventory.recipes.saveRecipe')}
                    </Button>
                </div>
            </CardContent>
        </Card>
